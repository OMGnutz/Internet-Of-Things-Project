...
namespace Admin_Dashboard
{
    public partial class LoginForm : Form
    {
        ...
        {
            if (tbusername.Text != "TestUser" && tbusername.Text != "Dexter" && tbusername.Text != "Donkey")
            {
            ...
            {
                SqlConnection myConnect = new SqlConnection(strConnectionString);
                string strCommandText = "SELECT Name, ID, Password FROM [User] WHERE Name=@uname AND Password = @pwd";
                SqlCommand cmd = new SqlCommand(strCommandText, myConnect);
                cmd.Parameters.AddWithValue("@uname", tbusername.Text);
                cmd.Parameters.AddWithValue("@pwd", tbpassword.Text);
